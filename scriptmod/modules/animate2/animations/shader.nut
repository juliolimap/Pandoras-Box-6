...
class ShaderAnimation extends Animation {
    ...
    function start() {
        base.start()
        if ( _param == null || typeof(_from) != "array" || typeof(_to) != "array" || _from.len() != _to.len() ) {
            print("incorrect ShaderAnimation properties specified, please provide param(\"name\") and arrays with matching number of values for `from()` and `to()`");
            return;
        }
    }
