...
// Os layouts de 'orbit' e 'grid' são bons exemplos do uso deste módulo
...
class ConveyorSlot
{
	...
	//
	// Essas funções podem ser substituídas por qualquer coisa
	// mais complicado (do que um único objeto de imagem)
	//
	function swap( other )
	{
		try
...
//
//Crie uma instância da classe Conveyor para gerenciar a navegação
//transições para uma série de objetos derivados de ConveyorSlot
//
//Use a função set_slots () para definir a matriz de objetos ConveyorSlot
//que o transportador deve gerenciar.
//
//Esses objetos ConveyorSlot serão distribuídos uniformemente sobre um
//"progresso" varia de 0,0 a 1,0. Em todos os casos em que os objetos de slot precisam
//a ser colocado, isso será feito chamando on_progress () do objeto slot
//função com o valor de "progresso" correspondente para o objeto.
//
//A função on_progress () é chamada para colocar os objetos ConveyorSlot ambos
//em sua posição de repouso, bem como durante cada quadro de uma transição.
//
///////////////////////////////////////////////////
class Conveyor
{
